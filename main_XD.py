...
# ══════════════════════════════════════════════════════════════
# CONFIG  (Linux paths — change if your folders differ)
# ══════════════════════════════════════════════════════════════
class Cfg:
    TRAIN_FEAT = r"/home/sabuj_khan/research/projects/Open_vocab/XDClipFeatures/XDTrainClipFeatures"
    TEST_FEAT  = r"/home/sabuj_khan/research/projects/Open_vocab/XDClipFeatures/XDTestClipFeatures"
    EXTRA_DIR  = r"/home/sabuj_khan/research/projects/Open_vocab/xd_extra"
    ...
    TRAM_TRAIN_DIR = r"/home/sabuj_khan/research/projects/DMN/tram_outputs_xd/train"

    FEAT_DIM = 512
    T = 256
    REPEAT = 16

    # XD VadCLIP-style backbone
    EMBED_DIM = 512
    VISUAL_WIDTH = 512
    VISUAL_HEAD = 1
    VISUAL_LAYERS = 1
    ...
    EPOCHS = 20
    BATCH_SIZE = 32          # start stable; can try 32 later
    LR = 5e-5
    WEIGHT_DECAY = 5e-4
    EVAL_EVERY = 30
    PATIENCE = 8
...
def get_tram_name(path_or_name):
    ...
    Keep full XD stem.
    Example:
      Bad.Boys.1995__#01-52-45_01-53-19_label_B2-0-0.npy
      -> Bad.Boys.1995__#01-52-45_01-53-19_label_B2-0-0.pt
    Only strip trailing feature chunk like __0 at the end.
    """
    name = os.path.basename(str(path_or_name))
    name = re.sub(r"\.(mp4|avi|mkv|npy|pt)$", "", name, flags=re.IGNORECASE)
...
class XDTrainDataset(Dataset):
    def __init__(self, csv_path, tram_dir, normal=True):
        ...
        if normal:
            df = df[df["label"] == "A"].reset_index(drop=True)
        else:
            df = df[df["label"] != "A"].reset_index(drop=True)

        valid = []
        for _, row in df.iterrows():
            tram_path = os.path.join(tram_dir, get_tram_name(row.get("path", row["fpath"])))
            if os.path.exists(tram_path) and os.path.exists(row["fpath"]):
                valid.append(row)

        self.df = pd.DataFrame(valid).reset_index(drop=True)
        self.tram_dir = tram_dir
        print(f"  {'Normal(A)' if normal else 'Anomaly'} with TRAM: {len(self.df)}")
    ...
    def __getitem__(self, idx):
        ...
        label = row["label"]
        is_anomaly = 0.0 if label == "A" else 1.0
...
# EVAL
# ══════════════════════════════════════════════════════════════
def evaluate(epoch, model, test_loader, device, best_ap, tag=""):
    model.eval()
    ...
    ap = average_precision_score(gt_bin, pred)
    auc = roc_auc_score(gt_bin, pred)

    marker = " *** NEW BEST ***" if ap > best_ap else ""
    prefix = f"[Test]{tag}" if tag else "[Test]"
    print(f"{prefix} Ep {epoch:02d} | AP:{ap:.4f} AUC:{auc:.4f}  BestAP:{max(best_ap, ap):.4f}{marker}")

    if ap > best_ap:
        os.makedirs("checkpoint", exist_ok=True)
        torch.save({"net": model.state_dict(), "ap": ap, "auc": auc},
                   "checkpoint/best_xd_vadclip_tram.pth")
    return ap

# ══════════════════════════════════════════════════════════════
# TRAIN
# ══════════════════════════════════════════════════════════════
def train_epoch(epoch, model, normal_loader, anomaly_loader, optimizer, device, test_loader, best_ap):
    model.train()
    prompt_text = get_prompt_text()
    ...
    for _ in range(n_steps):
        ...
        if steps % cfg.EVAL_EVERY == 0:
            ...
            ap = evaluate(epoch, model, test_loader, device, best_ap, tag=f" step:{steps}")
            if ap > best_ap:
                best_ap = ap
            model.train()
...
def main():
    ...
    print("XD-Violence | VadCLIP + TRAM soft + light RMTD | AP main")
    print("Loading datasets...")

    normal_ds  = XDTrainDataset(cfg.TRAIN_CSV, cfg.TRAM_TRAIN_DIR, normal=True)
    anomaly_ds = XDTrainDataset(cfg.TRAIN_CSV, cfg.TRAM_TRAIN_DIR, normal=False)
    test_ds    = XDTestDataset(cfg.TEST_CSV)
    print(f"Normal(A): {len(normal_ds)} | Anomaly: {len(anomaly_ds)}")

    if len(normal_ds) == 0 or len(anomaly_ds) == 0:
        print("ERROR: no matched feature+TRAM samples. Check paths / naming.")
        return
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        best_ap = train_epoch(epoch, model, normal_loader, anomaly_loader,
                              optimizer, cfg.DEVICE, test_loader, best_ap)
        ap = evaluate(epoch, model, test_loader, cfg.DEVICE, best_ap, tag=" EPOCH")
        if ap > best_ap:
            best_ap = ap
            no_improve = 0
        else:
        ...
        if no_improve >= cfg.PATIENCE:
            print(f"Early stop at epoch {epoch}. Best AP: {best_ap:.4f}")
            break
